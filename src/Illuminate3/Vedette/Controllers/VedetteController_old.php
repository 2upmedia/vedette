...
//		return View::make('frontend.auth.forgot-password-confirm');
    ...
    public function postIndex()
    {
        ...
        if ( $this->user->id )
        {
            ...
            return Redirect::route('user/login1112')
        ...
        if ($validator->passes())
        {
            ...
            if(!empty($password)) {
                if($password === $passwordConfirmation) {
                    ...
                    // Redirect to the new user page
                    return Redirect::route('users')->with('error', Lang::get('admin/users/messages.password_does_not_match'));
                }
            } else {
                unset($user->password);
...
return Redirect::intended(Config::get('vedette::vedette_settings.home_route'))->with('success', trans('lingos::auth.success.login'));
    ...
    public function postForgot()
    {
        ...
        {
            return Redirect::route('user/login11144')
    ...
    public function postReset()
    {
        ...
        {
            return Redirect::route('user/login1115')
            ->with( 'notice', Lang::get('confide::confide.alerts.password_reset') );
        }
        else
        {
            return Redirect::route('user/reset/'.$input['token'])
                ->withInput()
